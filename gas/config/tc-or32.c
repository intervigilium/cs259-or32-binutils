...
static unsigned int cpu_puf_key = 0;
...
#define OPTION_MPUFKEY (OPTION_MD_BASE + 0)

struct option md_longopts[] =
{
  { "mpufkey", required_argument, NULL, OPTION_MPUFKEY },
  ...
  switch (c)
    {
      case OPTION_MPUFKEY:
        ...
        if (!key || key > 0xffffffff)
          as_fatal (_("Invalid, out-of-range, or zero key: `%s'"), arg);
        cpu_puf_key = (unsigned int) (key & 0xffffffff);
        break;
      default:
        return 0;
    }
...
{
  fprintf (stream, _("\
  -mpufkey=KEY            cpu specific key generated by PUF hardware\n"));
}

